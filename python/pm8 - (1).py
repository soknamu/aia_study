...
input1 = Input(shape=(timesteps,6))
    ...
    for k in range(64):
        for i in range(120):
            if np.isnan(test_pm[j, 120*k+i, 1]) and i<84:
                test_pm[j, 120*k+i, 1] = model1.predict(np.concatenate([test_pm[j, 120*k+i-11:120*k+i-1, 1:], test_pm_aws[j, 120*k+i-11:120*k+i-1, :]], axis=1).reshape(-1,timesteps,6).astype(np.float32))
            elif i>=84:
                test_pm[j, 120*k+204-i-1, 1] = model2.predict(np.flip(np.concatenate([test_pm[j, 120*k+204-i:120*k+204-i+10, 1:], test_pm_aws[j, 120*k+204-i:120*k+204-i+10, :]], axis=1), axis=0).reshape(-1,timesteps,6).astype(np.float32))
...
submission['PM2.5']=l
submission.to_csv('C:/study/_save/finedust/Asus_Submit_15.csv')
ett = time.time()
print('걸린시간 :', np.round((ett-stt),2),'초')
model1.save("C:/study/_save/finedust/Aiur_Submit1.h5")
model2.save("C:/study/_save/finedust/Aiur_Submit2.h5")
