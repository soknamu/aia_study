...
x_augmented = train_datagen.flow(
    x_augmented, y_augmented, #y는 넣을 필요는 없는데 쌍으로 되있어서 넣음 (잘모름)
batch_size=augment_size, shuffle= False,
save_to_dir='d:/temp/' #사진이 40000개가 생성됨.
).next()[0] #했을 때 튜플이 나옴 즉, x_augmented[0]가 나옴 그래서 [0]을 붙혀줌. 그러면 x_augmented[0][0]가 나옴.
#증폭하는 부분.

ed = time.time() -st

print(augment_size,"개 증폭에 걸린시간 :", round(ed ,2) , '초')

#100 개 증폭에 걸린시간 : 0.22333192825317383 초
#100 개 증폭에 걸린시간 : 0.11 초
