...
def build_model(drop=0.5, optimizer='adam', activation='relu',
    ...
    x = Conv2D(32, (3, 3), activation=activation, padding='same')(inputs)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    x = Conv2D(64, (3, 3), activation=activation, padding='same')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    x = Conv2D(128, (3, 3), activation=activation, padding='same')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    x = Flatten()(x)
...
mcp = ModelCheckpoint(monitor='acc', mode = 'auto',
                      verbose=1,
                      save_best_only=True,
                      filepath='./_save/MCP/keras짱짱맨2.hdf5')
...
# 걸린시간 :  117.19870162010193
# Best Score:  0.9893166720867157
# Best Params:  {'optimizer': 'adam', 'node3': 32, 'node2': 256, 'node1': 64, 'lr': 0.1, 'drop': 0.3, 'batch_size': 100, 'activation': 'relu'}
# Best estimator <keras.wrappers.scikit_learn.KerasClassifier object at 0x00000195DBBEE4C0>
# 100/100 [==============================] - 0s 2ms/step - loss: 0.0347 - acc: 0.9937
# model Score:  0.9937000274658203
# acc : 0.9937