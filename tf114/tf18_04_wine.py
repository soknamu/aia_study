...
x_place = tf.compat.v1.placeholder(tf.float32, shape=[None, 13])
y_place = tf.compat.v1.placeholder(tf.int32, shape=[None, 1])

w = tf.compat.v1.Variable(tf.random.normal([13, 1], dtype=tf.float32), name='weight')
...
# 4. 평가, 예측
x_test_place = tf.compat.v1.placeholder(tf.float32, shape=[None, 13])
